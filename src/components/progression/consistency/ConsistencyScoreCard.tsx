type TierTone = 'poor' | 'erratic' | 'steady' | 'rock-solid'

const TIER_STYLE: Record<TierTone, { bg: string; border: string; text: string }> = {
  'poor':       { bg: 'bg-rose-500/10',    border: 'border-rose-500/40',    text: 'text-rose-300' },
  'erratic':    { bg: 'bg-amber-500/10',   border: 'border-amber-500/40',   text: 'text-amber-300' },
  'steady':     { bg: 'bg-emerald-500/10', border: 'border-emerald-500/40', text: 'text-emerald-300' },
  'rock-solid': { bg: 'bg-amber-400/10',   border: 'border-amber-400/40',   text: 'text-amber-200' },
}
...
export function ConsistencyScoreCard({
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Consistency Score</h2>
      <div className={`rounded-lg border p-8 text-center ${style.bg} ${style.border}`}>
        ...
        {lowConfidence && totalMatches > 0 && (
          <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-amber-500/40 bg-amber-500/10 px-3 py-1 text-[11px] text-amber-300">
            ...
            <span className="text-muted-foreground">— {totalMatches}/10 games needed</span>
          </div>
        )}
      </div>
    </section>
