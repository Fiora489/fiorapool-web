type TierTone = 'unaware' | 'developing' | 'aware' | 'hawk-eye'

const TIER_STYLE: Record<TierTone, { bg: string; border: string; text: string }> = {
  unaware:    { bg: 'bg-rose-500/10',    border: 'border-rose-500/40',    text: 'text-rose-300' },
  developing: { bg: 'bg-amber-500/10',   border: 'border-amber-500/40',   text: 'text-amber-300' },
  aware:      { bg: 'bg-emerald-500/10', border: 'border-emerald-500/40', text: 'text-emerald-300' },
  'hawk-eye': { bg: 'bg-amber-400/10',   border: 'border-amber-400/40',   text: 'text-amber-200' },
}

export function AwarenessScoreCard({
  score,
  tier,
  tierTone,
  lowConfidence,
  ...
  return (
    <section className="space-y-3">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-muted-foreground">Map Awareness</h2>
